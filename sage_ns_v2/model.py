import torch
import torch.nn as nn
import torch.nn.functional as F
import dgl
import dgl.nn.pytorch as dglnn
import dgl.function as fn
from torch.utils.data import IterableDataset, DataLoader
import random
import numpy as np
       
    
class BatchSampler:
    def __init__(self, g, fanouts, hard_neg_ratio):
        self.g = g
        self.fanouts = fanouts
        self.hard_neg_ratio = hard_neg_ratio
        self.neg_weights = g.ndata['neg_weight']
        
    def sample_blocks(self, seeds, heads=None, tails=None, neg_tails=None, use_all=False):
        blocks = []
        for index, fanout in enumerate(self.fanouts): 
            if index == 0 and use_all:
                fanout = -1
            frontier = dgl.sampling.sample_neighbors(self.g, seeds, fanout, replace=True)
            if heads is not None:
                eids = frontier.edge_ids(torch.cat([heads, heads]), torch.cat([tails, neg_tails]), return_uv=True)[2]
                if len(eids) > 0:
                    old_frontier = frontier
                    frontier = dgl.remove_edges(old_frontier, eids)
            
            block = dgl.to_block(frontier, seeds)
            seeds = block.srcdata[dgl.NID]
            blocks.insert(0, block)
        return blocks
    
    def sample_from_pairs(self, heads, tails, neg_tails):
        pos_graph = dgl.graph((heads, tails), num_nodes=self.g.number_of_nodes())
        neg_graph = dgl.graph((heads, neg_tails), num_nodes=self.g.number_of_nodes())
        pos_graph, neg_graph = dgl.compact_graphs([pos_graph, neg_graph])
        seeds = pos_graph.ndata[dgl.NID]
        blocks = self.sample_blocks(seeds, heads=heads, tails=tails, neg_tails=neg_tails)
        return pos_graph, neg_graph, blocks
    
    def gen_neg_tails(self, heads):
        if random.random() < self.hard_neg_ratio:
            walk_num = random.choice([3,4,5])
            neg_tails = dgl.sampling.random_walk(self.g, heads, length=walk_num)[0][:, -1]
#             neg_tails = dgl.sampling.node2vec_random_walk(self.g, heads, walk_length=walk_num, p=2, q=0.25)[:, -1]
        else:
            neg_tails = torch.multinomial(self.neg_weights, len(heads), replacement=True)
        return neg_tails
    
    def collate_fn_train(self, batches):
        heads = torch.stack(batches, dim=-1)
        tails = dgl.sampling.random_walk(self.g, heads, length=1)[0][:, -1]
        neg_tails = self.gen_neg_tails(heads)
        mask = (tails != -1) & (neg_tails != -1)
        pos_graph, neg_graph, blocks = self.sample_from_pairs(heads[mask], tails[mask], neg_tails[mask])
        return pos_graph, neg_graph, blocks
    
    def collate_fn_test(self, batches):
        heads, tails = torch.stack(batches, dim=-1)
        neg_tails = self.gen_neg_tails(heads)
        mask = (neg_tails != -1)
        pos_graph, neg_graph, blocks = self.sample_from_pairs(heads[mask], tails[mask], neg_tails[mask])
        return pos_graph, neg_graph, blocks
    
    def collate_fn_export(self, seeds):
        blocks = self.sample_blocks(seeds, use_all=True)
        return blocks
    
    
class SAGEModel(nn.Module):
    def __init__(self, g, feat_dim_dict):
        super().__init__()
        self.g = g
        self.feat_dim_dict = feat_dim_dict
        self.input_modules = self.init_input_modules()
        self.sage = SAGENet(feat_dim_dict)
        
    def forward(self, pos_graph, neg_graph, blocks):
        h_item = self.get_node_emb(blocks)
        pos_score = self.cal_pair_score(pos_graph, h_item)
        neg_score = self.cal_pair_score(neg_graph, h_item)
        return pos_score, neg_score
    
    def get_node_emb(self, blocks):
        h_item = self.get_input_emb(blocks[0].srcdata)
        h_item_dst = self.get_input_emb(blocks[-1].dstdata)
        return self.sage(blocks, h_item, h_item_dst)   
    
    def init_input_modules(self):
        module_dict = nn.ModuleDict()
        for column, data in self.g.ndata.items():
            if column not in self.feat_dim_dict:
                print("node feat", column, "not exist")
                continue

            feat_dim = self.feat_dim_dict[column]
            if data.dtype == torch.float32 or data.dtype == torch.float64:
                assert data.ndim == 2
                m = nn.Linear(data.shape[1], feat_dim)
                nn.init.xavier_uniform_(m.weight)
                nn.init.constant_(m.bias, 0)
                module_dict[column] = m
            elif data.dtype == torch.int32 or data.dtype == torch.int64:
                assert data.ndim == 1
                m = nn.Embedding(data.max() + 2, feat_dim, padding_idx=-1)
                nn.init.xavier_uniform_(m.weight)
                module_dict[column] = m
                
        return module_dict
    
    def get_input_emb(self, ndata):
        projections = []
        for col, data in ndata.items():
            if col not in self.feat_dim_dict:
                continue
            
            module = self.input_modules[col]
            result = module(data)
            projections.append(result)

        return torch.cat(projections, dim=-1)
    
    def cal_pair_score(self, pair_graph, h):
        with pair_graph.local_scope():
            pair_graph.ndata['h'] = h
            pair_graph.apply_edges(fn.u_dot_v('h', 'h', 's'))
            pair_score = pair_graph.edata['s'].view(-1)
            
        return pair_score


class SAGENet(nn.Module):
    def __init__(self, feat_dim_dict):
        super().__init__()
        feat_dim_sum = sum(feat_dim_dict.values())
        self.layers = nn.ModuleList()
        self.layers.append(dglnn.SAGEConv(feat_dim_sum, feat_dim_sum*2, 'pool'))
        self.layers.append(dglnn.SAGEConv(feat_dim_sum*2, feat_dim_sum, 'pool'))
        self.dropout = nn.Dropout(0.3)
        
    def forward(self, blocks, h_item, h_item_dst):
        h = h_item
        h = self.dropout(h)
        for l, (layer, block) in enumerate(zip(self.layers, blocks)):
            h = layer(block, h)
            if l != len(self.layers) - 1:
                h = F.relu(h)
                h = self.dropout(h)
                
        h = h / torch.linalg.norm(h, dim=-1, keepdim=True)
        return h
    